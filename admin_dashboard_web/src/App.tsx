    ...
    <main className="admin-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div><p className="eyebrow">Admin Dashboard</p><h1>{profile?.full_name ?? 'Masuk Dashboard Owner'}</h1></div>
        ...
        {profile && activeTab === 'overview' ? <Overview metrics={metrics} data={data} onOpenOperations={() => setActiveTab('operations')} /> : null}
        {profile && activeTab === 'operations' ? <OperationsPanel view={operationView} setView={setOperationView} requests={data.requests} orders={filteredOrders} inventory={data.inventory} inventoryMovements={data.inventoryMovements} loading={loading} onReview={reviewRequest} onAddInventory={() => setInventoryEditor(true)} onAdjustStock={setStockAdjuster} onUpdateOrder={updateOrderStatus} onRecordPayment={recordPayment} /> : null}
        {profile && activeTab === 'team' ? <TeamPanel view={teamView} setView={setTeamView} employees={filteredEmployees} shifts={data.shifts} onAdd={() => setEmployeeEditor('new')} onEdit={setEmployeeEditor} onAddShift={() => setShiftEditor('new')} onEditShift={setShiftEditor} /> : null}
        {profile && activeTab === 'reports' ? <ReportsPanel cash={data.cash} metrics={metrics} auditLogs={data.auditLogs} onAddCash={() => setCashEditor(true)} onExport={exportCashCsv} /> : null}
        {profile && activeTab === 'settings' ? <SettingsPanel shop={data.shop} loading={loading} onSave={saveShop} /> : null}
      </section>
