...
const demoData: DashboardData = {
  ...
  customers: [{ id: 'cus-1', name: 'Budi Santoso', phone: '081234567800', created_at: new Date().toISOString() }],
  inventory: [
...
function App() {
  ...
  async function loadDashboard(shopId: string) {
    const [employees, requests, orders, cash, customers, inventory, shifts, inventoryMovements, auditLogs, shop] = await Promise.all([
      ...
      supabase.from('customers').select('id, name, phone, created_at').eq('shop_id', shopId).order('created_at', { ascending: false }).limit(50),
  ...
  async function saveEmployee(values: EmployeeFormValues) {
    ...
    if (employeeEditor === 'new') {
      ...
      else { setEmployeeEditor(null); await loadDashboard(profile.shop_id) }
    } else if (employeeEditor) {
      const { error } = await supabase.from('employees').update({ name: values.name, phone: values.phone, position: values.position, shift_start: values.shift_start, shift_end: values.shift_end, late_tolerance_minutes: values.late_tolerance_minutes, is_active: values.is_active }).eq('id', employeeEditor.id).eq('shop_id', profile.shop_id)
      if (!error) await supabase.from('profiles').update({ full_name: values.name, phone: values.phone, is_active: values.is_active }).eq('employee_id', employeeEditor.id).eq('shop_id', profile.shop_id)
      if (error) setMessage(error.message); else { setEmployeeEditor(null); await loadDashboard(profile.shop_id) }
    }
    setLoading(false)
  }
  ...
  return (
    <main className="admin-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-card"><ShieldCheck size={18} /><div><strong>{profile ? 'Tersambung Supabase' : 'Mode demo'}</strong><span>{profile ? 'Data aman dibatasi per toko.' : 'Masuk untuk mengelola data asli.'}</span></div></div>
      </aside>

      <section className="workspace">
        <header className="topbar">
          <div><p className="eyebrow">Admin Dashboard</p><h1>{profile?.full_name ?? 'Owner Idola'}</h1></div>
          <div className="topbar-actions">
            ...
            <button className="icon-button" type="button" onClick={() => profile && void loadDashboard(profile.shop_id)} aria-label="Refresh data"><RefreshCw size={18} /></button>
            {profile ? <button className="ghost-button" type="button" onClick={signOut}><LogOut size={18} />Keluar</button> : null}
          </div>
        </header>

        {!profile ? <LoginPanel email={email} password={password} loading={loading} onEmail={setEmail} onPassword={setPassword} onSubmit={signIn} /> : null}
...
function ApprovalPanel({ requests, loading, onReview }: { requests: EmployeeRequest[]; loading: boolean; onReview: (request: EmployeeRequest, status: 'approved' | 'rejected' | 'completed' | 'paid') => Promise<void> }) {
  ...
  const shown = requests.filter((item) => filter === 'all' || item.status === filter)
  return <section className="wide-panel"><PanelHeader title="Approval Center" description="Semua request stok, lembur, izin, tukar shift, insentif, dan kasbon ada di sini." /><div className="filter-row">{[['pending', 'Menunggu'], ['approved', 'Disetujui'], ['completed', 'Selesai'], ['rejected', 'Ditolak'], ['all', 'Semua']].map(([id, label]) => <button key={id} type="button" className={filter === id ? 'active' : ''} onClick={() => setFilter(id)}>{label}</button>)}</div><div className="request-stack">{shown.map((request) => <article className="approval-card" key={request.id}><div><strong>{request.type}</strong><span>{request.employee_name} - {dateLabel(request.created_at)}</span><p>{request.reason}</p>{request.amount > 0 ? <b>{request.type.includes('Stok') ? `${request.amount} unit` : rupiah(request.amount)}</b> : null}{request.review_note ? <small>Catatan: {request.review_note}</small> : null}</div><div className="approval-actions"><StatusBadge label={request.status} />{request.status === 'pending' ? <><button className="approve" disabled={loading} type="button" onClick={() => void onReview(request, 'approved')}><CheckCircle2 size={16} />Setujui</button><button className="reject" disabled={loading} type="button" onClick={() => void onReview(request, 'rejected')}><XCircle size={16} />Tolak</button></> : null}{request.status === 'approved' ? <button className="approve" disabled={loading} type="button" onClick={() => void onReview(request, request.type.includes('Kasbon') || request.type.includes('Insentif') ? 'paid' : 'completed')}><CheckCircle2 size={16} />{request.type.includes('Kasbon') || request.type.includes('Insentif') ? 'Bayar' : 'Selesaikan'}</button> : null}</div></article>)}{!shown.length ? <div className="empty-state">Tidak ada request pada status ini.</div> : null}</div></section>
}
