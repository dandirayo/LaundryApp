alter table public.orders
  ...
  add column if not exists received_by_name_snapshot text not null default '';

with first_receivers as (
  select distinct on (payment.order_id)
    payment.order_id,
    payment.created_by,
    employee.name
  from public.payments as payment
  join public.employees as employee on employee.id = payment.created_by
  where payment.created_by is not null
  order by payment.order_id, payment.created_at
)
update public.orders as laundry_order
set received_by_employee_id = first_receiver.created_by,
    received_by_name_snapshot = first_receiver.name
from first_receivers as first_receiver
where first_receiver.order_id = laundry_order.id
  and laundry_order.received_by_name_snapshot = '';
...
revoke execute on function public.create_laundry_order(
